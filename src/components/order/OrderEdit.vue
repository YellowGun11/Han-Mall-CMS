          ...
          <el-row :gutter="10">
            ...
            >
              ...
              >
                <input
                  class="infor-value"
                  v-model="orderDetail[0].address"
                >
              </el-col>
            </el-col>
          </el-row>
          <el-footer>
            <el-button type="primary" @click="returnOrder">返回</el-button>
            <el-button type="success" @click="editOrder">确定</el-button>
          </el-footer>
        </el-tab-pane>
        <el-tab-pane
          label="商品信息"
...
export default {
  ...
  methods: {
    ...
    editOrder() {
      let arr=[]
      let orderNum = this.orderDetail[0].order_id;
      let consignee = this.orderDetail[0].user;
      arr.push(consignee)
      let tel = this.orderDetail[0].tel;
      arr.push(tel)
      let address = this.orderDetail[0].address;
      arr.push(address)
      let detail_addr=arr.join('-')
      this.$axios({
        url: "/editOrder",
        method: "post",
        data: {
    ...
    editDetail(index, row) {
      console.log(index);
      console.log(row);
      let orderNum = row.order_id;
      let goodsId = row.goods_id;
      let size = row.goods_size;
      this.$axios({
        url: "/editDetail",
        method: "post",
        data: {
